...
多选操作由 `FileSelection` 保存源服务与不可变条目列表，两栏共享文件剪贴板。`transferSelection` 预检查目标重名和复制到自身；同一远程服务的剪切使用 SFTP rename，其他移动先传输，再对比源文件列表、大小、修改时间与目标文件大小，通过后才删除源条目（不是内容哈希校验）。失败时保留未完成的剪贴板条目。手机挂载目录变化时清空剪贴板，批量传输期间禁止重新挂载。

批量粘贴不复用未知来源的旧分块临时文件，避免不同源的同名文件混合后误删原件；原有单文件传输的断点续传行为保持不变。同一主机资料且连接端点/用户/路由一致的不同标签，也参与“复制到自身”检查。
...
`SftpEditor` 捕获打开文件时的服务，保存失败保留编辑缓冲区；行号与文本共享垂直滚动，文本可水平滚动。`CodeTextController` 提供轻量语法着色，不是完整语言解析器，超过 20 万字符或输入法正在组合文本时不高亮。`AppSettings.sftpSyntaxHighlight` 控制开关，默认开启。
