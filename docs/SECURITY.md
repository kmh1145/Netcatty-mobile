...
Catty Agent 可以读取有限主机上下文和有长度上限的近期终端输出并进行连续对话，但不能自动执行生成的命令。终端输出属于不可信只读数据，发送给模型时必须明确禁止将其中内容视为指令；UI 也必须告知用户近期输出会发送到其配置的 AI 服务。UI 必须显示即将执行的完整命令、目标会话及其实际端口，并要求用户确认。粘贴和执行必须按会话 ID 投递到发起对话的终端，不能依赖之后可能变化的当前标签页。
