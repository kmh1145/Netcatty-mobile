...
- 服务器到服务器递归传输：同服务器优先内部复制/移动；不同服务器优先 SFTP 直传，不满足直连或认证条件时，经确认改用手机中转
- 文件和目录的上传、下载、分享、重命名、删除与新建，支持修改 Unix 权限和复制完整路径
- 可从远程目录一键跳转到对应 SSH 终端，并自动执行安全转义后的 `cd`
- 长按文件或通过“更多 → 多选”批量选择文件/目录，支持同服务器复制、剪切、粘贴和跨设备移动；移动确认后才执行，传输失败时保留源文件，同名冲突不会直接覆盖
- 多选压缩：服务器侧支持 TAR.GZ / ZIP（需安装 tar / zip）；手机侧流式生成 ZIP（暂不支持 ZIP64，单文件及整个压缩包须小于 4 GiB）
- 远程文本文件编辑支持与正文同步缩放的行号、自动换行、双指缩放字体、撤销/重做、未保存修改提示和代码高亮；可在打开的文件中按普通文本、全词或正则表达式搜索，支持区分大小写及上/下匹配导航；高亮可在设置中关闭，超过 20 万字符时自动暂停高亮
- 远程压缩文件可通过“解压缩”菜单在服务器上解压到新目录，支持 ZIP、TAR、TGZ、TAR.GZ、TAR.BZ2、TAR.XZ、7z、RAR（需服务器安装对应工具）；手机文件不提供此操作
- 显示传输百分比、已传输/总大小和实时速度
- 服务器直传使用源服务器已有的默认 SSH 密钥及可信主机指纹，不上传手机密码/私钥；需源端安装 OpenSSH `sftp`、目标端支持执行 `mv -nT`。手机代理/跳板机配置不自动迁移到服务器，直传受限时可选择手机中转。服务器内部操作不预扫描整个目录大小，显示操作阶段；大文件复制可读取目标临时文件大小显示进度
- Android 通过 Storage Access Framework 挂载用户选择的目录并持久保存权限
- iOS 使用“文件”App 中的“我的 iPhone/iPad > Netcatty”目录

### 系统管理

- **性能监控**：SSH 连接建立后即开始采集并缓存数据；点击 CPU、内存和根分区卡片可通过过渡动画展开每核心占用/CPU 型号、内存与 Swap、各挂载分区使用详情；网络区展示带时间轴的最近约 3 分钟吞吐曲线，按住或拖动可查看具体采样时间与收发速度，并显示累计收发流量和已建立 TCP 连接数（累计值来自服务器网络接口，不是当前 SSH 会话流量）。
...
- Catty Agent 支持流式聊天与停止、多服务商/模型切换、自动拉取模型和思考强度；面板可拖动至全屏，聊天历史本机保存，长对话自动摘要。可控制终端输出上传、预览上下文、隐藏身份和脱敏；命令确认后在当前终端执行，输出可预览后交给 AI 分析。首次使用有风险提示，新增配置和历史不进入云同步。[使用与安全边界](docs/AI_ASSISTANT.md)
