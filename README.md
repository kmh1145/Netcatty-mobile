...
- Catty Agent 支持流式聊天与停止、多服务商/模型切换、自动拉取模型和思考强度；聊天历史可在本机保存，长对话自动摘要。可预览并编辑待发送的终端上下文、隐藏服务器身份和脱敏；命令确认执行后，可选择把结果交给 AI 继续分析。新增配置和历史不进入云同步。[使用与安全边界](docs/AI_ASSISTANT.md)
