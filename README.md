...
- **为触屏设计的终端**：支持同一服务器多个标签页、分屏、全屏、可调文本选区与复制、中文输入、系统安全键盘、可作用于软键盘的 Ctrl/Alt/Shift、自定义快捷键、画中画和 Android 前台服务保活。
...
- **与桌面端兼容的保险库**：支持 WebDAV、GitHub OAuth/Gist 和 S3 云同步，可选开启与桌面端相同思路的自动同步；使用记录级修订、删除墓碑与写入前版本校验避免多设备覆盖和数据复活，未知字段无损保留，并可直接查看本地与云端同步版本。
...
- Catty Agent 支持多模型切换和最多 30 条对话历史；用户可选择是否附带有长度上限的近期终端输出，建议命令执行前必须确认
